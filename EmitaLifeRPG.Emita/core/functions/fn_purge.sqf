...
private["_message"];if(__GETC__(life_adminlevel) < 2) exitWith {closeDialog 0; 
hint "You need to be an Admin to use this function.";};
if(life_event) exitWith {closeDialog 0;
hint "There is already a server event under-way.";};
life_event = true;
...
sleep 180;
sleep 180;
sleep 180;     //For some reason, anything over 250 wouldn't work, so i split them up.sleep 180;sleep 180;
sleep 180;
sleep 180;
sleep 180;
sleep 180;
sleep 180;
_message = "The Purge will only last for another 30 Minutes!";
[[_message,name player,6],"TON_fnc_clientMessage",civilian,false] spawn life_fnc_MP;
sleep 150;
sleep 150;
_message = "We hope you are enjoying yourself";
[[_message,name player,6],"TON_fnc_clientMessage",civilian,false] spawn life_fnc_MP;
